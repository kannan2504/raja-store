...
import { serializeProductImage } from '../services/productImageStorage'
...
export class MongoProductRepository implements ProductRepository {
  async getCatalogProducts(): Promise<CatalogProduct[]> { return (await ProductModel.find({ active: true }).sort({ createdAt: -1 }).lean()).map((product) => ({ id: product.productId, name: product.name, slug: product.slug, description: product.description, shortDescription: product.shortDescription, price: product.price, compareAtPrice: product.compareAtPrice ?? undefined, images: (product.images ?? []).map((image) => serializeProductImage(image, product.productId)), category: product.category as { slug: string; name: string }, stock: product.stock, sku: product.sku, variants: (product.variants ?? []).map((variant) => ({ id: variant.id, sku: variant.sku, label: variant.label, price: variant.price ?? undefined, stock: variant.stock, options: variant.options })), isActive: product.active, rating: product.rating ?? undefined, isNew: product.isNew ?? undefined, isBestSeller: product.isBestSeller ?? undefined, tone: product.tone ?? undefined })) }
